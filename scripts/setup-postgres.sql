...
-- Nothing demo-specific goes below this line, or anywhere above it.
--
-- CI runs this file against a database holding only public.events and
-- public.users, with ON_ERROR_STOP=1, to prove the Postgres boundary holds
-- with no broker in the path. A GRANT or CREATE FUNCTION naming a schema that
-- exists only in demo/pocketos fails on the first statement and takes that
-- proof down with it - which is what happened on 2026-08-31, on the commit
-- that added them.
--
-- The pocketos demo's grants and its taper_add_column function live in
-- demo/pocketos/seed/03-broker.sql, where they are replayed with the schemas
-- they depend on and asserted after every reset.
-- verified-by: tests/test_integration.py::TestSetupSqlStaysGeneric
